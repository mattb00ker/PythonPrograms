...
def drawBoard(board):
    ...
    hline = '   ' # Initial space for the number down the left side of the board
    for i in range(1, 6):
        hline += (' ' * 9) + str(i)
        
    # Print the number across the top
    print hline
    print '   ' + ('0123456789' *6)
    print
...
def getNewBoard():
    ...
    board = []
    for x in range (60): #each list in the main list has 15 single-character strings
        board.append([])
...
def makeMove(board, chests, x, y):
    ...
    #  From the chests list as they are found. Return Flase if this is an invalid move.
...
def enterPlayerMove():
    # Let the player type in there move. Return a two-item list of int xy coorinates.
...
def playAgain():
    # This function returns True if the player wants to play again, otherwise if returns False.
...
while True:
    ...
    while sonarDevices > 0:
        ...
        print 'You have %s sonar devices%s left. %s treasure chests%s remaining.' % (sonarDevices, extraSsonar, len(theChests), extraSchest)
        print theChests
        x, y = enterPlayerMove()
    ...
    if sonarDevices == 0:
        ...
        print 'for home with treasure still out there! Game over.'
        print'    The reamining chests were here:'
